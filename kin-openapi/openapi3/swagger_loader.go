...
func failedToResolveRefFragmentPart(value string, what string) error {
	fmt.Printf("failed to resolve %q in fragment in URI: %q\n", what, value)
	return nil
...
func (swaggerLoader *SwaggerLoader) resolveComponent(swagger *Swagger, ref string, path *url.URL) (
	...
	parsedURL, err := url.Parse(ref)
	if err != nil {
		//fmt.Errorf("cannot parse reference: %q: %v", ref, parsedURL)
		return nil, nil, nil
	}
	...
	for _, pathPart := range strings.Split(fragment[1:], "/") {
		...
		if cursor, err = drillIntoSwaggerField(cursor, pathPart); err != nil {
			//fmt.Errorf("failed to resolve %q in fragment in URI: %q: %v", ref, pathPart, err.Error())
			return nil, nil, nil
		}
		if cursor == nil {
			//failedToResolveRefFragmentPart(ref, pathPart)
			return nil, nil, nil
		}
	}
...
func (swaggerLoader *SwaggerLoader) resolveRefSwagger(swagger *Swagger, ref string, path *url.URL) (*Swagger, string, *url.URL, error) {
	...
	if !strings.HasPrefix(ref, "#") {
		if !swaggerLoader.IsExternalRefsAllowed {
			//fmt.Errorf("encountered non-allowed external reference: %q", ref)
			return nil, "", nil, nil
		}
		parsedURL, err := url.Parse(ref)
		if err != nil {
		...
		if err != nil {
			//fmt.Errorf("error resolving path: %v", err)
			return nil, "", nil, nil
		}

		if swagger, err = swaggerLoader.loadSwaggerFromURIInternal(resolvedPath); err != nil {
			//fmt.Errorf("error resolving reference %q: %v", ref, err)
			return nil, "", nil, nil
		}
...
func (swaggerLoader *SwaggerLoader) resolveSchemaRef(swagger *Swagger, component *SchemaRef, documentPath *url.URL) error {
	...
	if component == nil {
		fmt.Printf("[WARNING] Invalid openapi schema: value MUST be a JSON object to be added\n")
		return nil
	}
...
func (swaggerLoader *SwaggerLoader) resolveLinkRef(swagger *Swagger, component *LinkRef, path *url.URL) error {
	...
	if component == nil {
		...
		fmt.Printf("[WARNING] Invalid link: value MUST be a JSON object to be added\n")
		return nil
	}
	if ref := component.Ref; ref != "" {
		if isSingleRefElement(ref) {
		...
		if isSingleRefElement(ref) {
			...
			*pathItem = p
			...
			if !strings.HasPrefix(ref, prefix) {
				...
				fmt.Printf("expected prefix %q in URI %q\n", prefix, ref)
				return
			}
